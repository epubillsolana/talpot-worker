  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/analyze') {
      try {
        ...
        if (!b.transcription || b.transcription.trim() === '') {
          return new Response(JSON.stringify({ ok: false, error: 'Transcripcion buida' }), {
        ...
        if (!d.choices || !d.choices[0]) {
          throw new Error('GPT no ha retornat resposta');
        }
        ...
        try {
          ...
          throw new Error('JSON invalid de GPT: ' + raw.substring(0, 100));
        }
